...
## 🛠️ Technical Notes
- **Identity Flow:** Ory Kratos login → session token → Token Exchange to JWT (watch for Kratos OIDC Token Exchange complexity — Session 13 risk).
- **Token Storage:** HttpOnly Cookie (prevents XSS token theft).
- **Gateway Role:** KrakenD acts as Traffic Coordinator — checks App **Scopes** (Gate 1) via Native Validator JSON config. No LUA scripts unless strictly necessary. No Go Plugins.
