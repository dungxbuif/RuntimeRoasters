...
## 📋 Tasks
- [ ] Triển khai `GinAuthMiddleware` trong `pkg/base/auth/middleware.go`:
    - Trích xuất Bearer token.
    - Parse & Verify RS256 signature bằng `jwksCache`.
    - Validate `exp`, `iss`, `aud`.
    - Inject `Claims` vào request context.
- [ ] Triển khai `GRPCUnaryInterceptor` trong `pkg/base/auth/interceptor.go`:
    - Trích xuất token từ gRPC Metadata.
    - Logic verify tương tự HTTP.
- [ ] Ghi `user.id` vào OTel span attributes cho cả hai loại middleware.
