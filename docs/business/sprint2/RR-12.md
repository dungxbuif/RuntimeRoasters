...
- **Authorization Model:** Hybrid — Casbin handles **RBAC** (role check, access decision) + Repository handles **ABAC** (ownership/data scoping via SQL filter).
- **Casbin Model:** RBAC with Role Hierarchy (`g` definition in `model.conf`).
- **Policy Storage:** CSV file for dev; Postgres adapter for production.
- **Scoping Claims Helper:** Centralized `pkg/base/casbin` helper translates policy outcomes into typed SQL filter structs — no scattered raw SQL in handlers.
- **Package:** `pkg/base/casbin` — enforcer factory, middleware, ScopeFilter builder.
- **Two-Gate context:** Gate 1 (App Scopes) is KrakenD's job (RR-10). Gate 2 (User Roles) is this ticket's job. They are independent checks on different claim types.
