...
This note captures implementation facts discovered during local Sprint 1-10 backend work. Keep it aligned with `AGENT.md` when architecture or operating decisions change.
...
- REST business services now follow the same Two-Gate auth direction:
  - Gate 1: KrakenD validates RS256 JWTs for protected REST endpoints using Hydra JWKS and issuer `http://localhost:4444/`.
  - Gate 2: retail/payment/logistics/trace/audit verify JWT locally and enforce service-local Casbin policies from the auth-service snapshot and `auth.policy.changed`.
  - Payment provider webhook routes remain public at the route-auth layer because they represent simulated external provider callbacks; payment-service still validates the demo provider contract.
- Sprints 6, 7, 8, 9, and 10 are connected as one backend demo flow:
  - retail-service creates the order SAGA.
  - warehouse-service reserves stock.
  - payment-service simulates Stripe/VNPay.
  - logistics-service assigns and delivers shipments.
  - trace-service stores Postgres events and an Elasticsearch read model.
  - audit-service writes immutable logs to Cassandra with Postgres fallback.
- Kafka demo consumers start at latest offset to avoid accidental historical backfill. Use `deployments/reset-demo-state.sh` before a clean SAGA run.
...
- New REST services initially exposed read/control endpoints without service-local auth. Shared HTTP auth/Casbin middleware now lives under `pkg/base/auth/transport/http`, `pkg/base/casbin/transport/http`, and `pkg/base/security`.
- KrakenD previously forwarded sprint 6-10 REST endpoints without JWT validation. Protected endpoints now carry `auth/validator`; login accept and simulated provider webhooks remain public.
...
- Protected REST endpoints should return `401` when called through KrakenD without a Bearer token.
