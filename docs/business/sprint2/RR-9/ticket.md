...
- **Database:** Postgres (dedicated `identity_db` và `hydra_db`).
- **Key Algorithm:** RS256 (Asymmetric Key Pair). Hydra quản lý Signing Keys; Public Keys distributed via JWKS.
- **S2S Security:** JWKS endpoint của Hydra được bảo vệ bởi `X-Internal-Secret` (Sprint 2 MVP).
