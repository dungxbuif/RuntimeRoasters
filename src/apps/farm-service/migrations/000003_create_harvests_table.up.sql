...
INSERT INTO farms (name, location, latitude, longitude, area, coffee_type, owner_id)
SELECT seed.name, seed.location::location_enum, seed.latitude, seed.longitude, seed.area, seed.coffee_type::coffee_type_enum, seed.owner_id::uuid
FROM (VALUES
    ...
    ('Chu Se Estate', 'PLEIKU', 14.0100, 108.0400, 30.0, 'ROBUSTA', '00000000-0000-0000-0000-000000000003')
) AS seed(name, location, latitude, longitude, area, coffee_type, owner_id)
WHERE NOT EXISTS (
    SELECT 1 FROM farms WHERE farms.name = seed.name
);
