...
- **Real-time Geo-Tracking:** Live GPS management using `Valkey` (high-performance `Redis` alternative).
...
#### Gateway Offloading & Zero Trust

The `API Gateway` acts as a centralized security hub, offloading JWT validation via `Ory Kratos` and injecting identity headers (`X-User-ID`, etc.). Communication between services is secured via `mTLS`, ensuring a `Zero Trust` internal network.
...
- **Theo dõi GPS thời gian thực:** Sử dụng `Valkey` (GEO commands, high-performance `Redis` alternative).
...
API Gateway đóng vai trò trung tâm bảo mật, thực hiện xác thực JWT qua `Ory Kratos` và inject identity headers (`X-User-ID`,...). Giao tiếp giữa các service nội bộ được bảo vệ bởi `mTLS`, đảm bảo mạng nội bộ `Zero Trust`.
