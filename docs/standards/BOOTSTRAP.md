...
- **Day 8**: Store completes consumer sale for demo cups/order items; UI-issued cup IDs are persisted, retail inventory decreases, and public QR trace documents become resolvable.

### Public QR Seed Requirement

Public QR demo seed data must represent **retail-sold cups/order items**, not generic product cards.

Required seeded relationships:

- retail product metadata for the SKU.
- retail inventory lot at the store, linked to upstream delivered order, warehouse, batch, and harvest lineage where known.
- retail stock movement rows showing received stock and sold stock.
- retail sale and retail sale item rows with stable UI-issued `cup_id` values.
- trace-service Postgres and Elasticsearch public documents resolvable by each `cup_id`/`trace_code`.

Seed invariants:

- every public `cup_id`/`trace_code` maps to exactly one sold sale item.
- every sold sale item references a retail inventory lot.
- backend derives origin from `product_id` plus the selected retail inventory lot lineage.
- retail inventory balance equals received quantity minus sale movement quantity.
- public trace data is sanitized and must not expose internal user, payment, session, or raw event payload details.
...
- **Retail Service**: retail products, inventory lots, stock movements, sales, and sale items for public QR demo state.
