...
Role dashboards must be work surfaces, not showcase-only pages:

- Role-aware navigation must expose only the role's operational duties.
- Status pipeline components should be used for long-running harvest, shipment, order, and sale flows.
- Realtime notifications should be visible for Warehouse Manager, Store Manager, Farm Manager, and Driver workflows when backend events exist.
- Driver route simulation UI must clearly post to backend logistics APIs; visual-only route movement is not valid evidence.
- Public QR trace pages must be no-auth, mobile-friendly, sanitized, and tied to sold `product_id` records.
- Store sold-cup lists must show invoice/sale item, product, sold time, and QR trace link without exposing payment internals.

See `docs/requirements/USER_STORIES.md` for active role coverage.
