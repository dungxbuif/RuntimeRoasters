...
### Local K8s Readiness Scope

Before a real cluster deployment, the repo must become deployable as container artifacts:

- production Dockerfiles for Go services and `client-app`.
- image naming controlled by `IMAGE_REGISTRY`, `IMAGE_TAG`, and `SERVICE_NAME`.
- one environment contract per service, separating ConfigMap values from Secret values.
- KrakenD/Ory configs parameterized for Kubernetes service DNS, issuer, callback URLs, and CORS origins.
- migration jobs for service databases and identity stores.
- idempotent seed jobs for demo identities, OAuth client, policies, and demo resources.

The first K8s readiness scope excludes cluster provisioning, registry setup, ingress controller installation, DNS, TLS issuer setup, and storage class provisioning.
...
- all images build with the selected `IMAGE_TAG`.
- service containers boot without repo-local `.env` files.
- health endpoints pass for every service: `/health/live` and `/health/ready`.
- KrakenD routes to service containers.
- OIDC login/callback works through configured public URLs.
