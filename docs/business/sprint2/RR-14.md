...
- **Summary:** Triển khai cơ chế thu hồi Token (Blacklist) sử dụng Redis và lắng nghe Token Revocation events từ Identity Server để đảm bảo người dùng đã logout không thể sử dụng JWT cũ.
- **Priority:** `MEDIUM`
...
- **When:** The Auth Middleware checks the token's `jti` against the Redis Distributed Blacklist.
...
- **Then:** The token's `jti` is added to Redis with a TTL matching the token's remaining expiration time.
...
## 🛠️ Technical Notes
- **Storage:** Redis (Distributed Blacklist).
- **Key Pattern:** `blacklist:<jti>`.
- **TTL:** Phải khớp với thời gian còn lại của token (`exp` - `now`).
- **Middleware Integration:** Cập nhật `pkg/base/auth` middleware để thực hiện `EXISTS` check trong Redis cho mỗi request (phải cực nhanh).
